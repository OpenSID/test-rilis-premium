...
namespace Modules\DTSEN\App\Services;
...
use Modules\DTSEN\App\Models\Dtsen;
use Modules\DTSEN\App\Models\DtsenAnggota;
use Modules\DTSEN\App\Models\DtsenLampiran;
use Modules\DTSEN\App\Models\DtsenPengaturanProgram;
use App\Models\KIA;
...
class DTSENRegsosEk2022k
{
     ...
     * @param mixed $dtks
     */
    public function generateDefaultDtsen($dtks): Dtsen
    {
        $dtks->setAppends([
            'kepala_keluarga',
            'jumlah_keluarga',
        ...
        if ($ids_anggota->count() > $dtks->dtksAnggota->count()) {
            $existing_dtks_anggotas = DtsenAnggota::whereIn('id_penduduk', $ids_anggota);
        ...
        if ($dtks->jumlah_keluarga > 1) {
            $dtks->all_dtks_id = Dtsen::select('id', 'id_rtm', 'id_keluarga', 'versi_kuisioner')
                ->withOnly([
    ...
    public function form(Dtsen $dtks)
    {
        ...
        $data['dtks'] = $this->generateDefaultDtsen($dtks);
    ...
    public function cetakPreviewSingle(Dtsen $dtks): void
    {
    ...
    public function ekspor(): void
    {
        ...
        $dtks_v2 = Dtsen::whereNotNull('id_rtm')->where('versi_kuisioner', DtsenEnum::REGSOS_EK2022_K)->get();
     ...
     */
    public function syncronizeWithOpenSid(Dtsen $dtks): Dtsen
    {
        $dtks->load([
            'rtm',
            'rtm.kepalaKeluarga' => static function ($builder): void {
        ...
        foreach ($dtks->rtm->anggota as $agt) {
            ...
            $dtks_anggota = DtsenAnggota::where('id_penduduk', $agt->id)->first();
            if (! $dtks_anggota) {
                $dtks_anggota = new DtsenAnggota();
            }
            $usia_dinamis              = $agt->umur; // attribute
        ...
        // Panggil generateDefaultDtsen untuk memastikan id_keluarga di tabel dtks terisi
        $this->generateDefaultDtsen($dtks);
     ...
     */
    public function save(array $request, ?Dtsen $dtks = null): array
    {
        $tipe = [
            'bagian1',
            'bagian2',
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    public function remove(Dtsen $dtks, array $request): array
    {
        $tipe = [
            'lampiran',
        ];
    ...
    public function syncKetDemografi(DtsenAnggota $dtks_anggota, $agt, ?Penduduk $kepala_keluarga, array $ref_eloquent_collection): DtsenAnggota
    {
    ...
    public function syncPendidikan(DtsenAnggota $dtks_anggota, $agt, Collection $daftar_pendidikan): DtsenAnggota
    {
        // Setelah Tamat SD
    ...
    public function syncKetenagakerjaan(DtsenAnggota $dtks_anggota, $agt, Penduduk $kepala_keluarga, $ref_eloquent_collection): DtsenAnggota
    {
    ...
    public function syncKepemilikanUsaha(DtsenAnggota $dtks_anggota, $agt, Penduduk $kepala_keluarga, $ref_eloquent_collection): DtsenAnggota
    {
    ...
    public function syncKesehatan(DtsenAnggota $dtks_anggota, $agt, $daftar_sakit_menahun): DtsenAnggota
    {
    ...
    public function syncProgramPerlindunganSosial(DtsenAnggota $dtks_anggota, $agt, ?Penduduk $kepala_keluarga, $ref_eloquent_collection): DtsenAnggota
    {
        ...
        if ($pengaturan_programs->count() > 0) {
            ...
            // lakukan update
            if ($is_dirty) {
                DtsenAnggota::where('id', $dtks_anggota->id)->update($to_be_updated);
            }
        }
    ...
    public function syncKepesertaanProgramKeluarga(Dtsen $dtks): Dtsen
    {
        $pengaturan_programs = DtsenPengaturanProgram::where('versi_kuisioner', '2')
        ...
        if ($pengaturan_programs->count() > 0) {
            ...
            // lakukan update
            if ($to_be_updated !== []) {
                Dtsen::where('id', $dtks->id)->update($to_be_updated);
            }
        }
    ...
    {
        $semua_dtks = Dtsen::where('id_rtm', $dtks->id_rtm)->whereNotNull('id_keluarga')->get();

        if ($semua_dtks->count() != $dtks->jumlah_keluarga) {
            // lepas semua anggota
            DtsenAnggota::where('id_dtks', $dtks->id)->update(['id_dtks' => null]);
            ...
            foreach ($dtks->keluarga_in_rtm as $keluarga) {
                ...
                elseif (! $dtks_keluarga) {
                    $new_dtks = Dtsen::where('id_rtm', $dtks->id_rtm)->whereNull('id_keluarga')->first();
                    if ($new_dtks) {
            ...
            Dtsen::where('id_rtm', $dtks->id_rtm)
                ->whereNotIn('id_keluarga', $dtks->keluarga_in_rtm->pluck('id'))
                ->update(['id_keluarga' => null]);
        }
    }

    protected function generateCetakPdf(Dtsen $dtks, $preview = false)
    {
        ...
        $dtks      = $this->generateDefaultDtsen($dtks);
    ...
    protected function removeLampiran(Dtsen $dtks, array $request): array
    {
        ...
        $lampiran = DtsenLampiran::withCount('dtks')->where('id', $lampiran_id)->first();
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian1(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian2(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian3(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian5(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian6(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian7Upload(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian4Demografi(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian4Pendidikan(Dtsen $dtks, array $request): array
    {
        $message        = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian4Ketenagakerjaan(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian4KepemilikanUsaha(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian4Kesehatan(Dtsen $dtks, array $request): array
    {
        $message = [];
     ...
     * @return array['content' => '', 'header_code' => '']
     */
    protected function saveBagian4ProgramPerlindunganSosial(Dtsen $dtks, array $request): array
    {
        $message = [];
    ...
    {
        if ($dtks_or_dtks_anggota instanceof Dtsen) {
            $attribute_tersedia = Regsosek2022kEnum::getUsedFields()['dtks'];
        } elseif ($dtks_or_dtks_anggota instanceof DtsenAnggota) {
