                ...
                @if ($cek['status'])
                    ...
                    @if ($ektp)
                        ...
                        <div class="form-group" style="{{ jecho(ENVIRONMENT == 'development', false, 'width: 0; overflow: hidden;') }}">
                            <input name="tag" id="tag" autocomplete="off" placeholder="Tempelkan e-KTP Pada Card Reader" class="form-control required number" type="password" onkeypress="if (event.keyCode == 13){$('#'+'validasi').attr('action', '{{ ci_route('kehadiran.cek-ektp') }}');$('#'+'validasi').submit();}">
                        </div>
