			...
			<div class="box box-info">
				<div class="box-header with-border">
					<?php if ($ci->CI->cek_hak_akses('u')): ?>
						<a href="<?= site_url("{$ci->controller}/form"); ?>" title="Tambah Kategori <?= $tipe; ?> Baru" class="btn btn-social btn-flat bg-olive btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"><i class="fa fa-plus"></i> Tambah Kategori <?= $tipe; ?> Baru</a>
					<?php endif; ?>
								...
								<table class="table table-bordered dataTable table-striped table-hover tabel-daftar">
									<thead class="bg-gray disabled color-palette">
										<tr>
											...
											<th>No</th>
											<th>Aksi</th>
											<th><?= url_order($o, "{$ci->controller}/{$func}/{$p}", 1, "Kategori {$tipe}"); ?></th>
										...
										<?php if ($main): ?>
											<?php foreach ($main as $key => $data): ?>
												<tr>
													...
													<td class="aksi">
														<?php if ($ci->CI->cek_hak_akses('u')): ?>
															<a href="<?= site_url("{$ci->controller}/form/{$data['id']}")?>" class="btn bg-orange btn-flat btn-sm" title="Ubah Kategori <?= $tipe; ?>"><i class="fa fa-edit"></i></a>
														<?php endif; ?>
														<?php if ($ci->CI->cek_hak_akses('h')): ?>
