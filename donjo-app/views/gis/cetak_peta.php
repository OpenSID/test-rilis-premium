...
<table class="title" leaflet-browser-print-content width="100%" style="border: solid 1px grey; text-align: center;">
  ...
  <tr>
    ...
    <?php elseif (in_array($wilayah, [ucwords($ci->setting->sebutan_dusun), 'RW', 'RT'])): ?>
  ...
  <tr>
    <td>
      <?php if ($wilayah == $nama_wilayah): ?>
        <h5 class="title text-center">PEMERINTAH <?= strtoupper($ci->setting->sebutan_kabupaten)?></h5>
        <h5 class="title text-center"><?= strtoupper($wil_atas['nama_kabupaten'])?></h5>
        <h5 class="title text-center"><?= strtoupper($ci->setting->sebutan_kecamatan)?></h5>
        <h5 class="title text-center"><?= strtoupper($wil_atas['nama_kecamatan'])?></h5>
        <h5 class="title text-center"><?= strtoupper($ci->setting->sebutan_desa)?></h5>
  ...
  <tr>
    <td>
      <?php if ($wilayah == $nama_wilayah): ?>
        ...
        <h3 class="title text-center"><?= strtoupper($ci->setting->sebutan_desa)?></h3>
        <h3 class="title text-center"><?= strtoupper($wil_atas['nama_desa'])?></h3>
      <?php elseif ($wilayah == ucwords($ci->setting->sebutan_dusun)): ?>
        <h3 class="title text-center">PETA WILAYAH</h3>
        <h3 class="title text-center"><?= strtoupper($ci->setting->sebutan_dusun)?></h3>
        <h3 class="title text-center"><?= strtoupper($wil_ini['dusun'])?></h3>
      <?php elseif ($wilayah == 'RW'): ?>
        <h3 class="title text-center">PETA WILAYAH</h3>
        <h3 class="title text-center">RW <?= $wil_ini['rw']?></h3>
        <h3 class="title text-center"><?= strtoupper($ci->setting->sebutan_dusun)?> <?= strtoupper($wil_ini['dusun'])?></h3>
