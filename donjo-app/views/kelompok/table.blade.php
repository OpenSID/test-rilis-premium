					...
					<div id="bantuan" class="box box-info">
						...
						<div class="box-body no-padding">
							<ul class="nav nav-pills nav-stacked">
								...
								<?php endforeach; ?>
								<li>
									<a class="btn btn-flat bg-purple btn-sm" href="<?= site_url("{$ci->controller}_master/clear"); ?>"><i class="fa fa-plus"></i> Kelola Kategori <?= $tipe; ?></a>
								</li>
							</ul>
						</div>
					</div>
				...
				<div class="col-md-9">
					<div class="box box-info">
						<div class="box-header with-border">
							...
							<a href="<?= site_url("{$ci->controller}/dialog/unduh"); ?>" class="btn btn-social btn-flat bg-navy btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Unduh Data <?= $tipe; ?>"><i class="fa fa-download"></i> Unduh</a>
						...
						<div class="box-body">
							<div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
								<form id="mainform" name="mainform" method="post">
									<div class="row">
										<div class="col-sm-9">
                                            <select class="form-control input-sm" name="status_dasar" onchange="formAction('mainform', '<?=site_url("{$ci->controller}/filter/status_dasar"); ?>')">
                                                <option value="">Pilih Status</option>
                                                <option value="1" <?= selected(1, $status_dasar); ?>>Aktif</option>
                                                <option value="2" <?= selected(2, $status_dasar); ?>>Tidak Aktif</option>
                                            </select>
											<select class="form-control input-sm" name="filter" onchange="formAction('mainform', '<?= site_url("{$ci->controller}/filter/filter"); ?>')">
										...
										<table class="table table-bordered dataTable table-striped table-hover tabel-daftar">
											<thead class="bg-gray disabled color-palette">
												<tr>
													...
													<th>No</th>
													<th>Aksi</th>
													<th>Kode <?= $ci->controller ?></th>
													<th width="50%"><?= url_order($o, "{$ci->controller}/{$func}/{$p}", 1, "Nama {$ci->controller}"); ?></th>
													<th><?= url_order($o, "{$ci->controller}/{$func}/{$p}", 3, "Ketua {$ci->controller}"); ?></th>
													<th><?= url_order($o, "{$ci->controller}/{$func}/{$p}", 5, "Kategori {$ci->controller}"); ?></th>
