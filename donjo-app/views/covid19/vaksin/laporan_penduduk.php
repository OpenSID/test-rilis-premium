			...
			<div class="col-md-8 col-lg-9">
				<div class="box box-info">
					<div class="box-header with-border">
						...
						<a href="<?= site_url("{$ci->controller}/clear/laporan_penduduk"); ?>" class="btn btn-social btn-flat bg-purple btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"><i class="fa fa-refresh"></i>Bersihkan</a>
					</div>
					<div class="box-body">
						<div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
							<form id="mainform" name="mainform" method="post">
								<div class="row">
									<div class="col-sm-12">
										<select class="form-control input-sm" name="vaksin" onchange="formAction('mainform', '<?= site_url($ci->controller) . '/filter/vaksin/laporan_penduduk' ?>')" style="margin-bottom: 5px;">
										...
										<select class="form-control input-sm " name="dusun" onchange="formAction('mainform','<?= site_url("{$ci->controller}/filter/dusun/laporan_penduduk"); ?>')" style="margin-bottom: 5px;">
										...
										<select class="form-control input-sm " name="jenis_vaksin" onchange="formAction('mainform','<?= site_url("{$ci->controller}/filter/jenis_vaksin/laporan_penduduk"); ?>')" style="margin-bottom: 5px;">
										...
										<div class="input-group input-group-sm" style="margin-bottom: 5px;">
											...
											<input name="umur" id="umur" class="form-control ui-autocomplete-input" placeholder="Rentang Umur" title="Contoh : 20-30" type="text" value="<?= ($umur == 0) ? '' : $umur ?>" onkeypress="if (event.keyCode == 13){$('#'+'mainform').attr('action', '<?= site_url($ci->controller . '/filter/umur/laporan_penduduk'); ?>');$('#mainform').submit();}" autocomplete="off">
										</div>

									</div>
									<div class="col-sm-12">
										<?php $ci->load->view('covid19/vaksin/laporan_penduduk_table') ?>
									</div>
								</div>
							</form>
						</div>
...
<div id="cetakBox" class="modal fade" role="dialog" style="padding-top:30px;">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<?php $ci->load->view('global/dialog_cetak', ['form_action' => site_url($ci->controller . '/laporan_penduduk_cetak/cetak'), 'aksi' => 'Cetak']) ?>
		</div>
	</div>
</div>
...
<div id="unduhBox" class="modal fade" role="dialog" style="padding-top:30px;">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<?php $ci->load->view('global/dialog_cetak', ['form_action' => site_url($ci->controller . '/laporan_penduduk_cetak/unduh'), 'aksi' => 'Unduh']) ?>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.tgl-datepicker').on('dp.change', function(e) {
			formAction('mainform', '<?= site_url("{$ci->controller}/filter/tanggal_vaksin/laporan_penduduk"); ?>')
		});
