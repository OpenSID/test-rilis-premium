    ...
    */
    ...
    public function list_anggota($id = 0, $options = ['dengan_kk' => true], $nik_sementara = false)
    {
        $this->config_id('u')
            ...
            ->select("
                ...
                END) as status_kawin
            ")
            ->select(['b.dusun', 'b.rw', 'b.rt', 'x.nama as sex', 'u.kk_level', 'd.nama as pendidikan', 'd.id as pendidikan_id', 'f.nama as warganegara', 'g.nama as golongan_darah', 'h.nama AS hubungan', 'h.id AS hubungan_id', 'k.alamat', 'tc.nama AS cacat'])
    ...
    public function get_kepala_kk($id, $is_no_kk = false)
    {
        ...
        $this->config_id('u')
            ...
            ->select("(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(`tanggallahir`)), '%Y')+0) AS umur")
            ->select('d.nama as pendidikan, x.nama as sex')
