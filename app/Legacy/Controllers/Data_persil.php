...
class Data_persil extends \App\Legacy\Core\Admin_Controller
{
    private $set_page;
    ...
    public function simpan_persil($page = 1)
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            if (! $this->input->post('id_persil') && $cdesa_awal) {
                ci_redirect("cdesa/mutasi/{$cdesa_awal}/{$id_persil}");
            } else {
                ci_redirect('data_persil');
            }
        }
        ...
        ci_redirect('data_persil/form/' . $id);
    }
