...
class Cdesa extends \App\Legacy\Core\Admin_Controller
{
    ...
    public function simpan_cdesa($page = 1)
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            if ($this->input->post('id')) {
                ...
                ci_redirect("cdesa/create_mutasi/{$id_cdesa}");
            }
        } else {
            $_SESSION['success']   = -1;
            ...
            if ($jenis_pemilik == 1) {
                if ($id) {
                    ci_redirect('cdesa/create/edit/' . $id);
                } else {
                    ci_redirect('cdesa/create');
                }
            } else {
                if ($id) {
                    ci_redirect('cdesa/create/edit/' . $id);
    ...
    public function simpan_mutasi($id_cdesa, $id_mutasi = '')
    {
        ...
        if ($data['id_persil']) {
            ci_redirect("cdesa/mutasi/{$id_cdesa}/{$data['id_persil']}");
        } else {
            ci_redirect("cdesa/rincian/{$id_cdesa}");
        }
    }
    ...
    public function hapus_mutasi($cdesa, $id_mutasi)
    {
        ...
        $this->db->where('id', $id_mutasi)
            ->delete('mutasi_cdesa');
        ci_redirect("cdesa/mutasi/{$cdesa}/{$id_persil}");
    }
    ...
    public function awal_persil($id_cdesa, $id_persil, $hapus = false)
    {
        ...
        ci_redirect("cdesa/mutasi/{$id_cdesa}/{$id_persil}");
    }
}
