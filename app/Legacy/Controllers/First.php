...
class First extends \App\Legacy\Core\Web_Controller
{
    ...
    */
    public function artikel($thn, $bln, $hr, $url)
    {
        if (is_numeric($url)) {
            ...
            if ($data_artikel) {
                ...
                ci_redirect('artikel/' . buat_slug($data_artikel));
            }
        }
    ...
    {
        ci_redirect('data-kelompok/' . $slug);
    }

    public function suplemen($slug = '')
    {
        ci_redirect('data-suplemen/' . $slug);
    }
    ...
    public function add_comment($id = 0)
    {
        ...
        ci_redirect($_SERVER['HTTP_REFERER'] . '#kolom-komentar');
    }
