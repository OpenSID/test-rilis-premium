...
class Bumindes_tanah_kas_desa extends \App\Legacy\Core\Admin_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $order  = $this->tanah_kas_desa_model::ORDER_ABLE[$this->input->get('order[0][column]')];
    ...
    public function clear()
    {
        ...
        ci_redirect('bumindes_tanah_kas_desa');
    }
    ...
    public function add_tanah_kas_desa()
    {
        ...
        if ($this->session->success == -1) {
            ...
            ci_redirect('bumindes_tanah_kas_desa/form');
        } else {
            ci_redirect('bumindes_tanah_kas_desa/clear');
        }
    }
    ...
    public function update_tanah_kas_desa($id)
    {
        ...
        if ($this->session->success == -1) {
            $this->session->dari_internal = true;
            ci_redirect("bumindes_tanah_kas_desa/form/{$id}");
        } else {
            ci_redirect('bumindes_tanah_kas_desa/clear');
        }
    }
