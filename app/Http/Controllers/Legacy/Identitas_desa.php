...
class Identitas_desa extends \App\Legacy\Core\Admin_Controller
{
    ...
    public function form()
    {
        ...
        if ($main) {
            $form_action = ci_route('identitas_desa.update', $main->id);
        } else {
            $form_action = ci_route('identitas_desa.insert');
        }
    ...
    public function maps($tipe = 'kantor')
    {
        ...
        $data['breadcrumb']   = [
            ['link' => ci_route('identitas_desa'), 'judul' => 'Identitas ' . ucwords($this->setting->sebutan_desa)],
        ];
