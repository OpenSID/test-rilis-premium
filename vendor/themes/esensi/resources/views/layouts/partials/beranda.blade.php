...
<div class="container mx-auto lg:px-5 px-3 flex flex-col lg:flex-row my-5 gap-3 lg:gap-5 justify-between text-gray-600">
  <main class="lg:w-2/3 w-full overflow-hidden space-y-5">
    ...
    @if (empty($cari) && count($slider_gambar ?? []) > 0 && request()->segment(2) != 'kategori' && (request()->segment(2) !== 'index' && request()->segment(1) !== 'index'))
      @include($folder_themes . '.partials.slider')
    @endif
    ...
    @if (empty($cari) && count($slider_gambar ?? []) > 0 && request()->segment(2) != 'kategori' && (request()->segment(2) !== 'index' && request()->segment(1) !== 'index'))
      @include($folder_themes . '.partials.headline')
    @endif

    @if ($artikel)
      @foreach ($artikel as $post)
        @include($folder_themes . '.partials.article_list', ['post' => $post])
      @endforeach
      <div class="pagination space-y-1 flex-wrap w-full">
        @include($folder_themes . '.commons.paging', ['paging_page' => $paging_page])
      </div>
    @else
      @include($folder_themes . '.partials.empty_article', ['title' => $title])
    @endif
  </main>

  <!-- Bagian sidebar / widget -->
  <div class="lg:w-1/3 w-full">
    @include($folder_themes . '.partials.sidebar')
  </div>
</div>
